var obj = {
    ...
    drawTurntable: function (arr) {
        var len = this.list.length;
        var angle = 2 * Math.PI / len;
        this.turnContext.save();
        this.turnContext.translate(this.centerX, this.centerY);

        var cW = this.turntable.width;
        var cH = this.turntable.height;
        var arc = this.centerX;
        var startAngle=0;
        ...
        for (var i = 0; i < len; i++) {
            ...
            this.turnContext.fillStyle = "#E5302F";
            var text = arr[i];
            var textRadius = cW * .6;
            this.turnContext.save();
            this.turnContext.translate(cW / 2 + Math.cos(angle + arc / 2) * textRadius, cH / 2 + Math.sin(angle + arc / 2) * textRadius);
            this.turnContext.rotate(angle + arc / 2 + Math.PI / 2);
            this.turnContext.fillText(text, -this.turnContext.measureText(text).width / 2, 0);
